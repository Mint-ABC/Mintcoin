...
###   mintcoincore.org and mintcoin.org and compares them.
...
WORKINGDIR="/tmp/mintcoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://mintcoincore.org"
HOST2="https://mintcoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="mintcoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'mintcoin-',
...
if [ $? -ne 0 ]; then
   echo "mintcoin.org failed to provide signature file, but mintcoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "mintcoin.org and mintcoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Mintcoin Core binary release signing key installed?"
   fi
