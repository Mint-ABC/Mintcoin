...
const char * const MINTCOIN_CONF_FILENAME = "mintcoin.conf";
const char * const MINTCOIN_PID_FILENAME = "mintcoind.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "mintcoind -foo=bar
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Mintcoin";
#else
    // Unix
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", MINTCOIN_CONF_FILENAME);
...
fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", MINTCOIN_PID_FILENAME)));
}
